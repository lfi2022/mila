...
Rewards, payments and contributions now use typed feature API clients backed by Fastify, including Premium checkout/return, direct parent bank-transfer instructions and parent contribution history. The contribution form states that Mila charges no fee and never receives the funds. Parents confirm receipt manually; the full IBAN is never stored in browser configuration. General administration remains behind its transitional server adapter until Stage 19, and orders expose only domain types until Stage 17; no placeholder network call pretends those features exist. Public-list/reporting remains a server-only transitional Supabase consumer; affiliate redirects moved to Fastify in Stage 10.

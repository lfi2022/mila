...
export function RewardsPanel({ registryId }: { registryId: string }) {
  ...
  const redeemMutation = useMutation({
    mutationFn: (offerId?: string) => redeem({ data: { registryId, ...(offerId ? { offerId } : {}) } }),
    onSuccess: (result) => {
  ...
  if (rewards.isLoading) return <p className="text-sm text-muted-foreground">Chargement de vos récompenses…</p>;
  const data = rewards.data;
  if (!data) return <p className="text-sm text-muted-foreground">Récompenses indisponibles pour le moment.</p>;
    ...
    return (
      <div className="rounded-2xl border border-border bg-card p-6">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Le programme de récompenses n'est pas encore actif sur votre compte. Vous serez prévenus dès son ouverture.
        </p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-3">
        <div className="rounded-2xl border border-border bg-card p-5">
          ...
          <p className="mt-1 font-display text-3xl">{formatCents(data.availableCents, data.currency)}</p>
        </div>
        <div className="rounded-2xl border border-border bg-card p-5">
          <p className="text-sm text-muted-foreground">En attente de confirmation</p>
          <p className="mt-1 font-display text-3xl">{formatCents(data.pendingCents, data.currency)}</p>
        </div>
        <div className="rounded-2xl border border-border bg-card p-5">
          <p className="text-sm text-muted-foreground">Cumul gagné</p>
          <p className="mt-1 font-display text-3xl">{formatCents(data.lifetimeEarnedCents, data.currency)}</p>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-border bg-secondary/40 p-5 text-sm leading-relaxed text-muted-foreground">
        ...
        <p className="mt-2">
          Une récompense reste « en attente » tant que le marchand n'a pas définitivement validé l'achat. Elle devient
          disponible ensuite, et peut être annulée en cas de retour ou d'annulation de commande.
        </p>
      </div>
      ...
      <div className="rounded-2xl border border-border bg-card p-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          {data.redemptionEnabled && (offers.data?.offers.length ?? 0) === 0 && (
            <Button disabled={!canRedeem || redeemMutation.isPending} onClick={() => redeemMutation.mutate(undefined)}>
      ...
      <div className="rounded-2xl border border-border bg-card p-5">
        ...
        {data.transactions.length === 0 ? (
          <p className="mt-2 text-sm text-muted-foreground">
            Aucun mouvement pour l'instant. Vos récompenses arriveront quand vos proches achèteront des cadeaux depuis
            votre liste.
          </p>
        ) : (
              ...
              return (
                <li key={txn.id} className="flex flex-wrap items-center justify-between gap-2 py-3">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className={txn.amountCents < 0 ? "text-sm text-muted-foreground" : "text-sm font-medium"}>
                      {txn.amountCents > 0 ? "+" : ""}
          ...
          <ul className="mt-3 space-y-2 text-sm">
            {data.redemptions.map((r) => (
              <li key={r.id} className="flex items-center justify-between gap-3">
                <span>
                  {formatCents(r.amountCents, data.currency)} — {new Date(r.requestedAt).toLocaleDateString("fr-BE")}
                </span>
